...
func (p *process) Start() (err error) {
	...
	if err != nil {
		return common.NewErrorf("Failed to generate xray config file: %v", err)
	}
	configPath := GetConfigPath()
	err = os.WriteFile(configPath, data, fs.ModePerm)
	if err != nil {
		return common.NewErrorf("Failed to write config file: %v", err)
	}
...
func (p *process) GetTraffic(reset bool) ([]*Traffic, error) {
	if p.apiPort == 0 {
		return nil, common.NewError("xray api port error:", p.apiPort)
	}
